defmodule CaHeoShopWeb.UserAuth do
  use CaHeoShopWeb, :verified_routes
  ...
  @remember_me_cookie "_ca_heo_shop_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CaHeoShopWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      CaHeoShopWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule CaHeoShopWeb.PageLive do
        use CaHeoShopWeb, :live_view

        on_mount {CaHeoShopWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{CaHeoShopWeb.UserAuth, :require_authenticated}] do
